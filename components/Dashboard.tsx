...
const Dashboard: React.FC<DashboardProps> = ({ user, polls, announcements, tasks, notifications }) => {
  const stats = [
    { label: 'Votações Ativas', value: polls.filter(p => p.active).length, icon: 'fa-check-to-slot', color: 'bg-brand-1 dark:bg-brand-2' },
    { label: 'Avisos no Mural', value: announcements.length, icon: 'fa-bullhorn', color: 'bg-brand-2 dark:bg-brand-3' },
    { label: 'Chamados STAFF', value: tasks.filter(t => t.status !== 'Resolvido').length, icon: 'fa-screwdriver-wrench', color: 'bg-brand-3 dark:bg-brand-2' },
    { label: 'Mensagens Privadas', value: notifications.length, icon: 'fa-bell', color: 'bg-brand-4 dark:bg-brand-5 text-brand-1' },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((s, i) => (
          <div key={i} className="bg-white dark:bg-white/5 p-6 rounded-3xl border border-slate-100 dark:border-white/10 shadow-sm flex items-center gap-5 hover:shadow-md transition-all">
            <div className={`${s.color} w-14 h-14 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-brand-1/5`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white dark:bg-white/5 rounded-3xl border border-slate-100 dark:border-white/10 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-slate-50 dark:border-white/10 flex justify-between items-center bg-white dark:bg-transparent">
            ...
            <button className="text-brand-2 dark:text-brand-3 text-xs font-bold uppercase tracking-widest hover:text-brand-1">Ver Tudo</button>
          </div>
          <div className="divide-y divide-slate-50 dark:divide-white/10">
            {announcements.slice(0, 4).map(a => (
              <div key={a.id} className="p-6 hover:bg-neutral-surface dark:hover:bg-white/5 transition-colors cursor-pointer group">
        ...
        <div className="bg-white dark:bg-white/5 rounded-3xl border border-slate-100 dark:border-white/10 shadow-sm overflow-hidden h-fit">
          ...
          <div className="p-6 space-y-4">
            {user.role === UserRole.MORADOR ? (
              polls.filter(p => p.active && !p.votedUnits.includes(user.unit)).map(p => (
                <div key={p.id} className="bg-neutral-surface dark:bg-brand-2/10 p-5 rounded-2xl border border-brand-4 dark:border-brand-2/30">
                  ...
                  <button className="mt-4 w-full bg-brand-1 dark:bg-brand-3 text-white py-2.5 rounded-xl text-xs font-bold hover:bg-brand-2 transition-all shadow-md shadow-brand-1/10">
                    Votar Agora
                  </button>
                </div>
              ))
              ...
              tasks.filter(t => t.status !== 'Resolvido').slice(0, 3).map(t => (
                <div key={t.id} className="flex items-center gap-4 p-4 border-l-4 border-brand-2 bg-neutral-surface dark:bg-brand-2/10 rounded-r-2xl">
            ...
            <button className="w-full border-2 border-dashed border-slate-200 dark:border-white/10 text-slate-400 dark:text-brand-4 py-3 rounded-2xl text-xs font-bold hover:border-brand-3 hover:text-brand-3 transition-all">
